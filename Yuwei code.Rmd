...
# 2. data processing
## step 1 - transpose the fang file and merge
Here we transpose the fang file and merge with snp information according to SNP ID.
```{r}
library(dplyr)
t_fang <- t(fang)
...
## Step 2: Split the merged file ino 2 groups
...
## Step 3: split by chromosome and sequence by position
...
```
# 3. visualization
### Answer 1: SNPs per chromosome, we can take x = chromosome, y = snp position.
```{r}
library(ggplot2)
merge <- merge_group %>%
  t() %>%
...
p1 <- merge %>%
  ...
  geom_density(binwidth = 1e7, aes(fill = Chromosome)) +
...
p7 <- merge_group2 %>%
  ...
  ggplot(aes(x = as.factor(Chromosome), y = count, fill = candidate.random)) +
    geom_bar(stat = "identity") +  
